	...
	*m_pStartNum << 0;
	...
	if (!bFileMode) {
		...
		*m_pEndNum << nNum - 1;
		 ...
		 strcpy(m_VideoFilePath, openFileDialog.GetPath());
		 strcpy(m_VideoFileName, openFileDialog.GetFilename());

		 m_VideoProcessingVc.open(m_VideoFilePath);
		...
		if (m_pSaveFrameCheck->GetValue()) {
			if (m_nProcessingNum == 0) {
				...
				ss
					...
					<< std::setfill('0') << std::setw(2) << localTime.tm_sec << '_';
			...
			sprintf(SaveFileName, "%s/%sframe%05d.jpg", FolderName, m_strSaveFileNameHeader.c_str(), m_nProcessingNum);
		...
		if (m_VideoProcessingVc.isOpened()) {
			...
			if (m_pSaveFrameCheck->GetValue()) {
				if (m_nProcessingNum == 0) {
					...
					char VideoFileStem[256] = { 0 };
					strncpy(VideoFileStem, m_VideoFileName, strlen(m_VideoFileName) - 4);
				...
				sprintf(SaveFileName, "%s/%sframe%010d.jpg", FolderName, m_strSaveFileNameHeader.c_str(), m_nProcessingNum);
				cv::imwrite(SaveFileName, CurrentImage);
			}
